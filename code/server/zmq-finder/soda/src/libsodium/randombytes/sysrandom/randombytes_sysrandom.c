...
# define RtlGenRandom CryptGenRandom
...
//----
// http://sourceforge.net/p/mingw-w64/code-ironcrate/ci/master/tree/misc/ic_rand_s.c
...
if (!l_GenRandom)
      ...
      if (!hmod)
	{
	  ...
	  abort(); //return IC_EINVAL;
	}

      //f = (fRtlGenRandom) GetProcAddressA (hmod, "SystemFunction036");
      f = (fRtlGenRandom) GetProcAddress (hmod, "SystemFunction036");
      if (!f)
	{
	  //IC_INV_PARAM("Invalid arguments");
	  abort(); //return ic_errno;
	}
      l_GenRandom = f;
    }

    if ((*l_GenRandom) ((PVOID) buf, (ULONG) size)) {
        abort(); /* LCOV_EXCL_LINE */
    }
//----
